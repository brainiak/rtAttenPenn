...
class RtfMRIClient():
    """
    Class to handle parsing configuration and running the experiment logic
    """
    ...
    def run(self, cfg):
        ...
        self.sendExpectSuccess(
            MsgType.Command, MsgEvent.StartExperiment, cfg.experiment)
        self.id_fields.sessionId = cfg.session.sessionId
        self.sendExpectSuccess(
            MsgType.Command, MsgEvent.StartSession, cfg.session)
        for run in cfg.runs:
            ...
            for blockGroup in run.blockGroups:
                ...
                self.sendExpectSuccess(
                    MsgType.Command, MsgEvent.StartBlockGroup, blockGroup)
                for block in blockGroup.blocks:
                    self.id_fields.blockId = block.blockId
                    self.sendExpectSuccess(
                        MsgType.Command, MsgEvent.StartBlock, block)
                    ...
                    for trial_idx in range(trialRange[0], trialRange[1]):
                        ...
                        self.sendExpectSuccess(
                            MsgType.Command, MsgEvent.TrialData, trial)
                    del self.id_fields.trialId
                    self.sendExpectSuccess(
                        MsgType.Command, MsgEvent.EndBlock, block)
                del self.id_fields.blockId
                self.sendExpectSuccess(
                    MsgType.Command, MsgEvent.EndBlockGroup, blockGroup)
            del self.id_fields.blockGroupId
            self.sendExpectSuccess(MsgType.Command, MsgEvent.EndRun, run)
        del self.id_fields.runId
        self.sendExpectSuccess(
            MsgType.Command, MsgEvent.EndSession, cfg.session)
        del self.id_fields.sessionId
        self.sendExpectSuccess(
            MsgType.Command, MsgEvent.EndExperiment, cfg.experiment)
    ...
    for ridx, run in enumerate(cfg.runs):
        ...
        for bgidx, blockGroup in enumerate(run.blockGroups):
            if blockGroup.blockGroupId is None:
                raise ValidationError(
                    "blockGroupId not defined in BG[{}]".format(bgidx))
            ...
            for blkidx, block in enumerate(blockGroup.blocks):
                ...
                if block.trials is None:
                    raise ValidationError(
                        "trials not defined in Block[{}]".format(blkidx))
                trialRange = [int(x) for x in block.trials.split(':')]
